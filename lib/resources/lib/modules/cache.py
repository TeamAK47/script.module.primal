...
def _find_cache_version():
    ...
    try:
        curVersion = control.addon('script.module.primal').getAddonInfo('version')
...
def _find_cache_versionAlt(): #Added to keep track of plugin.video.primal version
    ...
    try:
        curVersion = control.addon('plugin.video.primal').getAddonInfo('version')
        if oldVersion != curVersion:
            with open(versionFile, 'wb') as fh: fh.write(curVersion)
            return True
        else: return False
